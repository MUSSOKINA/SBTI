# -*- coding: utf-8 -*-
"""
SBTI 人格测试 — Python 复刻版。
题库、计分、模板匹配、酒鬼分支、HHHH 兜底与 index.html 内联脚本保持一致。
答题仅按题目 id 存字典，与网页上题目展示顺序无关。
"""
...
# -------------------------------------------------------------------------
# 与 index.html 一致的数据（勿改文案与 value，以保证结果一致）
# -------------------------------------------------------------------------
...
TYPE_LIBRARY: Dict[str, Dict[str, str]] = {
    "CTRL": {
        "code": "CTRL",
        "cn": "拿捏者",
        "intro": "怎么样，被我拿捏了吧？",
...
# 与网页 TYPE_IMAGES 一致（Python 端仅作展示路径参考）
...
NORMAL_TYPES: List[Dict[str, str]] = [
...
def format_user_lm_pattern(levels: Dict[str, str]) -> str:
    """与 NORMAL_TYPES 相同格式：每三个字母一段，段间用「-」。"""
    parts: List[str] = []
    for i in range(0, 15, 3):
        chunk = "".join(levels[DIMENSION_ORDER[i + j]] for j in range(3))
        parts.append(chunk)
    return "-".join(parts)
...
# -------------------------------------------------------------------------
# 核心算法（对齐 Math.round / JS 排序）
# -------------------------------------------------------------------------


def js_math_round(x: float) -> int:
    """正数场景与 JavaScript Math.round 一致（0.5 向 +∞）。"""
    import math
...
def get_drunk_triggered(answers: Dict[str, Any]) -> bool:
    return answers.get(DRUNK_TRIGGER_QUESTION_ID) == 2


def compute_result(answers: Dict[str, Any]) -> Dict[str, Any]:
    """
    入参 answers: 题目 id -> 选项 value（int）。须包含全部 q1..q30；
    drink_gate_q1 若存在且为 3，则应有 drink_gate_q2（与网页一致）。
    特殊题不参与维度求和。
    """
    raw_scores: Dict[str, int] = {dim: 0 for dim in DIMENSION_META}
    for q in QUESTIONS:
        raw_scores[q["dim"]] += int(answers.get(q["id"]) or 0)

    levels: Dict[str, str] = {dim: sum_to_level(s) for dim, s in raw_scores.items()}
    user_vector = [level_num(levels[d]) for d in DIMENSION_ORDER]

    ranked: List[Dict[str, Any]] = []
    for t in NORMAL_TYPES:
        vector = [level_num(ch) for ch in parse_pattern(t["pattern"])]
        distance = 0
        exact = 0
        for i in range(len(vector)):
            diff = abs(user_vector[i] - vector[i])
            distance += diff
            if diff == 0:
                exact += 1
        similarity = max(0, js_math_round((1 - distance / 30) * 100))
        lib = TYPE_LIBRARY[t["code"]]
        ranked.append(
            {
                **t,
                **lib,
    ...
    ranked.sort(
        key=lambda x: (x["distance"], -x["exact"], -x["similarity"]),
    )

    best_normal = ranked[0]
    drunk_triggered = get_drunk_triggered(answers)
    ...
    if drunk_triggered:
        ...
        secondary_type = dict(best_normal)
        mode_kicker = "隐藏人格已激活"
        ...
        final_type = dict(best_normal)

    fun_note = (
        "本测试仅供娱乐。隐藏人格和傻乐兜底都属于作者故意埋的损招，请勿把它当成医学、心理学、相学、命理学或灵异学依据。"
        if special
        else "本测试仅供娱乐，别拿它当诊断、面试、相亲、分手、招魂、算命或人生判决书。你可以笑，但别太当真。"
    )
...
def format_result_text(result: Dict[str, Any]) -> str:
    ...
    for dim in DIMENSION_ORDER:
        ...
        lines.append(f"- {name}  {lv} / {sc}分  {expl}")
    lines.extend(["", "友情提示", result["fun_note"]])
    if TYPE_IMAGES.get(t["code"]):
        lines.extend(["", f"配图（与网页一致路径）: {TYPE_IMAGES[t['code']]}"])
    return "\n".join(lines)


def _prompt_choice(question: Dict[str, Any]) -> int:
    codes = ["A", "B", "C", "D", "E", "F"]
    print(question["text"])
    opts = question["options"]
    valid = {o["value"] for o in opts}
    for i, o in enumerate(opts):
        c = codes[i] if i < len(codes) else str(i + 1)
        print(f"  {c}. {o['label']}  (value={o['value']})")
    while True:
        raw = input("请输入选项字母或分值 (1-4): ").strip().upper()
        if not raw:
            continue
        if raw in codes[: len(opts)]:
            idx = codes.index(raw)
            return int(opts[idx]["value"])
        try:
            v = int(raw)
            if v in valid:
                return v
...
def run_interactive() -> None:
    print("SBTI 人格测试（Python，逻辑与 index.html 一致）\n")
    answers: Dict[str, Any] = {}
    for q in QUESTIONS:
        print(f"\n--- {q['id']} [{q['dim']}] ---")
        answers[q["id"]] = _prompt_choice(q)

    sq0 = SPECIAL_QUESTIONS[0]
    print(f"\n--- {sq0['id']} [补充题] ---")
    answers[sq0["id"]] = _prompt_choice(sq0)

    if answers[sq0["id"]] == 3:
        sq1 = SPECIAL_QUESTIONS[1]
        print(f"\n--- {sq1['id']} [补充题] ---")
        answers[sq1["id"]] = _prompt_choice(sq1)
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if argv and argv[0] in ("-h", "--help"):
        print(
            "用法:\n"
            "  python sbti_quiz.py              交互答题\n"
            "  python sbti_quiz.py --json 文件.json  从答案 JSON 计算（键为题目 id）\n"
            "  python sbti_quiz.py --self-test      运行内置对照测试\n"
        )
        return
    if argv and argv[0] == "--json" and len(argv) > 1:
...
def _self_test() -> None:
    """内置烟测：维度全 H、酒鬼强制、HHHH 兜底（与网页分支一致）。"""
    answers = {q["id"]: 3 for q in QUESTIONS}
    answers["drink_gate_q1"] = 1
    r = compute_result(answers)
    assert r["levels"]["S1"] == "H"
    assert r["final_type"]["code"] != "DRUNK"

    a2 = dict(answers)
    a2["drink_gate_q1"] = 3
    a2["drink_gate_q2"] = 2
    r2 = compute_result(a2)
    assert r2["final_type"]["code"] == "DRUNK"
    assert r2["badge"] == "匹配度 100% · 酒精异常因子已接管"

    # 下方答案组经随机搜索得到 best_normal.similarity==57（<60），应触发 HHHH
    a3 = {
        "q1": 1,
        "q2": 1,
        "q3": 1,
    ...
    r3 = compute_result(a3)
    assert r3["best_normal"]["similarity"] == 57
    assert r3["final_type"]["code"] == "HHHH"
    assert r3["mode_kicker"] == "系统强制兜底"
    print("self-test OK")
